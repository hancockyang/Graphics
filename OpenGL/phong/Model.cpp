...
Model::Model(glm::vec3&& scale, glm::vec3&& translation, glm::vec3&& rotation,
             std::string&& filepath)
    : m_scale{std::move(scale)}, m_translation{std::move(translation)}, m_rotation{std::move(rotation)}, m_modelmatrix { CalModelMatrix() }
{
...
glm::mat4 const Model::CalModelMatrix()
{
...
void Model::MoveModel(glm::vec3&& translation)
{
   m_translation += translation;
   m_modelmatrix = CalModelMatrix();
}

glm::vec3 const& Model::GetPosition()
{
...
{
    return std::make_unique<Model>(std::move(scale), std::move(translation), std::move(rotation), std::move(filepath));
